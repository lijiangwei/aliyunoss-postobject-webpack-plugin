...
class AliyunOSS {
    async uploadFile(outputPath, options) {
        let ossInfo = await this.getSignature(options.signatureUrl);
        if (ossInfo) {
    ...
    async uploadFileToOss(fileName, ossFileName, ossInfo) {
